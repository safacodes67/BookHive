...
export default function productMaker(bookData) {
  bookData.forEach((book) => {
	...
	<div class="thumb-content">
		...
		<p class="item-price"><strike class="">${lastPriceInRupees}</strike><b>₹${priceInRupees}</b></p>
  ...
  if (el.target.innerText == "Add to Cart ") {
    let data = {
      ...
      name: el.target.parentElement.parentElement.children[1].children[0]
        .innerText,
      price:
        el.target.parentElement.parentElement.children[1].children[2].children[1].innerText.replace(/[^0-9.]/g, ""),
      value: 1,
      lastprice: (() => {
        if (
